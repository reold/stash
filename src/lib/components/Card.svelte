<script lang="ts">
  ...
  import { cardHelper } from "$lib/card";
  ...
  let parsed = $derived.by(() => {
    ...
    let display = "";
    if (type == "number") display = `${number}`;
    else if (type == "plus2") display = "+2";
    else if (type == "plus4") display = "+4";
    else if (type == "reverse") display = "rev";

    let color = "nocolor";
    if (number == 15 || type != "plus4") color = cardHelper.parseColor(card);

    return { number, type, display, color };
  });
</script>
